...
serve(async (req) => {
  ...
  try {
    ...
    if (!authHeader?.startsWith('Bearer ')) {
      console.error('scrape-product-prices: No authorization header');
      return new Response(
    ...
    if (claimsError || !claimsData?.claims) {
      console.error('scrape-product-prices: Invalid token:', claimsError?.message);
      return new Response(
        JSON.stringify({ success: false, error: 'Unauthorized' }),
        { status: 401, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    ...
    console.log('scrape-product-prices: Authenticated user:', userId);
